...
for (const c of cases) {
  it(`${c.op} calls only the documented endpoint and maps its contract`, async () => {
    ...
    expect(url.origin + url.pathname).toBe(`https://graph.facebook.com/v26.0${c.path}`);
    ...
    expect(output).toEqual({ connector: "instagram", action: c.op, source: "provider", ...c.output });
...
it("declares complete manual token contracts and bound account ID", async () => {
  ...
  expect(manifest.auth.scopes).toEqual(["instagram_basic", "instagram_content_publish", "pages_read_engagement"]);
  ...
  expect(manifest.network.allowedHosts).toEqual(["graph.facebook.com"]);
});
...
it("returns empty final media page without fabricated cursors or credential-bearing URLs", async () => {
  ...
  expect(output).toEqual({ connector: "instagram", action: "media.list", source: "provider", media: [] });
});
...
it("read-only QA rejects fixture echoes, empty and missing provider identities", async () => {
  ...
  expect(qa.connector).toBe("instagram");
