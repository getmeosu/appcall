...
for (const scenario of qa.scenarios) {
  it(`${scenario.operation} requires a real read-only provider result`, async()=> {
    ...
    let calls=0;
    const output=await actions[scenario.operation]!({...scenario.input,accessToken:"test-token",fetch:async()=>{
...
it("rejects absent, empty and whitespace-only account identifiers",async()=>{
  ...
  const identityKey="account_id";
  for(const id of [undefined,"","   "]) {
    const response = {...fixture.response, [identityKey]:id};
    const output=await actions.healthcheck!({accessToken:"test-token",fetch:async()=>Response.json(response)});
    expect(scenario.expect.assertions.every(a=>accepts(output,a))).toBe(false);
  }
});
it("rejects a root listing missing its cursor or completion flag", async () => {
  const scenario=qa.scenarios.find(s=>s.operation==="root.list")!;
  for(const response of [{entries:[],has_more:false},{entries:[],cursor:"saved"},{entries:[],cursor:"",has_more:false}]) {
    const output=await actions["root.list"]!({accessToken:"test-token",fetch:async()=>Response.json(response)});
    expect(scenario.expect.assertions.every(a=>accepts(output,a))).toBe(false);
  }
});
