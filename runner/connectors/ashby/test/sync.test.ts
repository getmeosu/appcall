...
// Ashby API inside a try/catch that asserted nothing when no error was
...
describe("Ashby jobs.list sync", () => {
  test("requests the documented endpoint on the allowed host", async () => {
    const { calls, impl } = stubFetch('{"jobs":[{"id":"j1","title":"Engineer","jobUrl":"https://acme.example/j1"}]}');
  ...
  test("classifies an upstream failure instead of throwing a bare Error", async () => {
    ...
    await expect(executeJobsListSync({ ...{ boardName: "acme" }, fetch: impl }))
