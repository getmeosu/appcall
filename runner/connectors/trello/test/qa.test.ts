...
import fixture from "../fixtures/member_me.json";
...
it("provides a read-only identity probe whose contract rejects echoes and missing identities", async () => {
  ...
  expect(qa.connector).toBe("trello");
  ...
  const { id: omitted, ...missingIdentity } = output;
