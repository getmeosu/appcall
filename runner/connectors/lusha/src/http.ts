...
export function parseLushaRateLimit(status: number, headers: Record<string, string>): { limited: boolean; retryAfterSeconds: number } {
  if (status === 429) {
    const retryAfter = Number(headers["retry-after"] ?? headers["x-rate-limit-reset"] ?? "10");
    return { limited: true, retryAfterSeconds: retryAfter > 0 ? retryAfter : 10 };
  }
  return { limited: false, retryAfterSeconds: 0 };
}

export type LushaClientOptions = { apiKey: string; fetch?: typeof fetch; operation?: string };

export function createLushaClient(options: LushaClientOptions) {
  const operation = options.operation ?? "person.enrich";
  ...
  return {
    async fetchJSON(path: string, init: RequestInit = {}): Promise<{ status: number; headers: Record<string, string>; body: unknown }> {
      const response = await httpClient.fetchText(`https://api.lusha.com${path}`, {
        ...init,
        headers: {
...
export type LushaClient = ReturnType<typeof createLushaClient>;
