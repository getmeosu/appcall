...
import fixture from "../fixtures/me.json";
...
it("provides a read-only identity probe whose contract rejects echoes and missing identities", async () => {
  ...
  expect(qa.connector).toBe("figma");
  ...
  expect(assertions.some(a => a.path === "userId" && a.op === "type")).toBe(true);
  ...
  const { userId: omitted, ...missingIdentity } = output;
