...
export function parseApifyRateLimit(status: number, headers: Record<string, string>): { limited: boolean; retryAfterSeconds: number } {
  if (status === 429) {
...
export type ApifyClientOptions = { apiKey: string; fetch?: typeof fetch; operation?: string };

export function createApifyClient(options: ApifyClientOptions) {
  const operation = options.operation ?? "actors.list";
  ...
  return {
    async fetchJSON(path: string, init: RequestInit = {}): Promise<{ status: number; headers: Record<string, string>; body: unknown }> {
      const response = await httpClient.fetchText(`https://api.apify.com/v2${path}`, {
        ...init,
        headers: {
