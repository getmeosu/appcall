...
export function parseCalendlyRateLimit(status: number, headers: Record<string, string>): { limited: boolean; retryAfterSeconds: number } {
  if (status === 429) {
...
export type CalendlyClientOptions = { accessToken: string; fetch?: typeof fetch; operation?: string };

export function createCalendlyClient(options: CalendlyClientOptions) {
  const operation = options.operation ?? "scheduled_events.list";
  ...
  return {
    async fetchJSON(path: string, init: RequestInit = {}): Promise<{ status: number; headers: Record<string, string>; body: unknown }> {
      const response = await httpClient.fetchText(`https://api.calendly.com${path}`, {
...
export type CalendlyClient = ReturnType<typeof createCalendlyClient>;
