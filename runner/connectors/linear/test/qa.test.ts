...
import fixture from "../fixtures/viewer.json";
...
it("provides a read-only identity probe whose contract rejects echoes and missing identities", async () => {
  ...
  expect(qa.connector).toBe("linear");
  ...
  expect(assertions.some(a => a.path === "viewerId" && a.op === "type")).toBe(true);
  ...
  const { viewerId: omitted, ...missingIdentity } = output;
