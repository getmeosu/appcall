...
// Greenhouse API inside a try/catch that asserted nothing when no error was
...
describe("Greenhouse jobs.list sync", () => {
  test("requests the documented endpoint on the allowed host", async () => {
    const { calls, impl } = stubFetch('{"jobs":[{"id":1,"title":"Engineer","absolute_url":"https://acme.example/j1"}]}');
  ...
  test("classifies an upstream failure instead of throwing a bare Error", async () => {
    ...
    await expect(executeJobsListSync({ ...{ boardToken: "acme" }, fetch: impl }))
