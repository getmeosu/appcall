...
import fixture from "../fixtures/user_current.json";
...
it("provides a read-only identity probe whose contract rejects echoes and missing identities", async () => {
  ...
  expect(qa.connector).toBe("confluence");
  ...
  expect(assertions.some(a => a.path === "accountId" && a.op === "type")).toBe(true);
  const { actions } = compileDeclarativeConnector(manifest as never);
  let calls = 0;
  const output = await actions.healthcheck!({
    ...{"site": "acme.atlassian.net", "email": "qa@example.com", "apiToken": "test-token", "basicAuth": "cWE6dGVzdA=="},
    fetch: async () => {
      calls += 1;
  ...
  const { accountId: omitted, ...missingIdentity } = output;
